...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        //数据库方言配置
        private static readonly Dictionary<Type, SortedDictionary<DBDialectOption, string>> _dictDBDialect = new Dictionary<Type, SortedDictionary<DBDialectOption, string>>();

        public enum DBDialectOption
        {
            Encapsulation,
            IdentitySql,
            PagedListSql,
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetDialect(IDbConnection connection, DBDialectOption dialectOption)
        {
            return _dictDBDialect[connection.GetType()][dialectOption];
        }
        ...
        {
            _dictDBDialect.Clear();

            //SQLServer
            var sqlServer = new SortedDictionary<DBDialectOption, string>() {
                {DBDialectOption.Encapsulation,  "[{0}]"},
                {DBDialectOption.IdentitySql,  string.Format("SELECT CAST(SCOPE_IDENTITY()  AS BIGINT) AS [id]")},
                {DBDialectOption.PagedListSql,  "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY {OrderBy}) AS PagedNumber, {SelectColumns} FROM {TableName} {WhereClause}) AS u WHERE PagedNumber BETWEEN (({PageNumber}-1) * {RowsPerPage} + 1) AND ({PageNumber} * {RowsPerPage})"},
                {DBDialectOption.ParamPrefix,  "@"}
            };
            _dictDBDialect.Add(typeof(System.Data.SqlClient.SqlConnection), sqlServer);

            //Oracle
            var oracle = new SortedDictionary<DBDialectOption, string>() {
                {DBDialectOption.Encapsulation,  "\"{0}\""},
                {DBDialectOption.IdentitySql,  ""},
                {DBDialectOption.PagedListSql,  "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY {OrderBy}) AS PagedNumber, {SelectColumns} FROM {TableName} {WhereClause}) U WHERE PagedNumber BETWEEN (({PageNumber}-1) * {RowsPerPage} + 1) AND ({PageNumber} * {RowsPerPage})"},
                {DBDialectOption.ParamPrefix,  ":"}
            };
            ...
            //    {DBDialectOption.Encapsulation,  "\"{0}\""},
            //    {DBDialectOption.IdentitySql,  string.Format("SELECT LAST_INSERT_ROWID() AS id") },
            //    {DBDialectOption.PagedListSql,  "Select {SelectColumns} from {TableName} {WhereClause} Order By {OrderBy} LIMIT {RowsPerPage} OFFSET (({PageNumber}-1) * {RowsPerPage})"},
            //    {DBDialectOption.ParamPrefix,  "@"}
            //};
            //_dictDBDialect.Add(typeof(SQLiteConnection), sqlLite);

            //PostgreSQL
            //var postgreSQL = new SortedDictionary<DBDialectOption, string>() {
            //    {DBDialectOption.Encapsulation,  "\"{0}\""},
            //    {DBDialectOption.IdentitySql,  string.Format("SELECT LASTVAL() AS id") },
            //    {DBDialectOption.PagedListSql,  "Select {SelectColumns} from {TableName} {WhereClause} Order By {OrderBy} LIMIT {RowsPerPage} OFFSET (({PageNumber}-1) * {RowsPerPage})"},
            //    {DBDialectOption.ParamPrefix,  "@"}
            //};
            //_dictDBDialect.Add(typeof(PostgreSQLConnection), postgreSQL);
        ...
        public static T Get<T>(this IDbConnection connection, object id, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            var name = GetTableName(currenttype, connection);
            var sb = new StringBuilder();
            sb.Append("Select ");
            //create a new empty instance of the type to get the base properties
            BuildSelect(sb, GetScaffoldableProperties<T>().ToArray(), connection);
            ...
            for (var i = 0; i < idProps.Count; i++)
            {
                ...
                sb.AppendFormat("{0} = {1}{2}", GetColumnName(idProps[i], connection), GetDialect(connection, DBDialectOption.ParamPrefix), idProps[i].Name);
            }
        ...
        public static IEnumerable<T> GetList<T>(this IDbConnection connection, object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            {
                sb.Append(" where ");
                BuildWhere<T>(sb, whereprops, connection, whereConditions);
            }
        ...
        {
            if (string.IsNullOrEmpty(GetDialect(connection, DBDialectOption.PagedListSql)))
            ...
            var name = GetTableName(currenttype, connection);
            var sb = new StringBuilder();
            var query = GetDialect(connection, DBDialectOption.PagedListSql);
            if (string.IsNullOrEmpty(orderby))
            {
                orderby = GetColumnName(idProps.First(), connection);
            }
        ...
        public static TKey Insert<TKey, TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            var name = GetTableName(entityToInsert, connection);
            var sb = new StringBuilder();
            sb.AppendFormat("insert into {0}", name);
            sb.Append(" (");
            BuildInsertParameters<TEntity>(sb, connection);
            sb.Append(") ");
            sb.Append("values");
            sb.Append(" (");
            BuildInsertValues<TEntity>(sb, connection);
            sb.Append(")");
            ...
            {
                sb.Append(";" + GetDialect(connection, DBDialectOption.IdentitySql));
            }
            else
            {
                keyHasPredefinedValue = true;
        ...
        public static int Update<TEntity>(this IDbConnection connection, TEntity entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            StringBuilderCache(masterSb, $"{typeof(TEntity).FullName}_Update", sb =>
            {
                ...
                var name = GetTableName(entityToUpdate, connection);

                sb.AppendFormat("update {0}", name);

                sb.AppendFormat(" set ");
                BuildUpdateSet(entityToUpdate, sb, connection);
                sb.Append(" where ");
                BuildWhere<TEntity>(sb, idProps, connection, entityToUpdate);
        ...
        public static int Delete<T>(this IDbConnection connection, T entityToDelete, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            StringBuilderCache(masterSb, $"{typeof(T).FullName}_Delete", sb =>
            {
                ...
                var name = GetTableName(entityToDelete, connection);

                sb.AppendFormat("delete from {0}", name);

                sb.Append(" where ");
                BuildWhere<T>(sb, idProps, connection, entityToDelete);
        ...
        public static int DeleteList<T>(this IDbConnection connection, object whereConditions, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            StringBuilderCache(masterSb, $"{typeof(T).FullName}_DeleteWhere{whereConditions?.GetType()?.FullName}", sb =>
            {
                ...
                {
                    sb.Append(" where ");
                    BuildWhere<T>(sb, whereprops, connection);
                }
        ...
        private static void BuildUpdateSet<T>(T entityToUpdate, StringBuilder masterSb, IDbConnection connection)
        {
            StringBuilderCache(masterSb, $"{typeof(T).FullName}_BuildUpdateSet", sb =>
            {
                ...
                for (var i = 0; i < nonIdProps.Length; i++)
                {
                    ...
                    sb.AppendFormat("{0} = {1}{2}", GetColumnName(property, connection), GetDialect(connection, DBDialectOption.ParamPrefix), property.Name);
        ...
        private static void BuildSelect(StringBuilder masterSb, IEnumerable<PropertyInfo> props, IDbConnection connection)
        {
            StringBuilderCache(masterSb, $"{props.CacheKey()}_BuildSelect", sb =>
            {
                ...
                for (var i = 0; i < propertyInfos.Count(); i++)
                {
                    ...
                    if (addedAny)
                        sb.Append(",");
                    sb.Append(GetColumnName(property, connection));
                    ...
                    if (property.GetCustomAttributes(true).SingleOrDefault(attr => attr.GetType().Name == typeof(ColumnAttribute).Name) != null)
                        sb.Append(" as " + Encapsulate(property.Name, connection));
                    addedAny = true;
                }
            });
        }

        private static void BuildWhere<TEntity>(StringBuilder sb, IEnumerable<PropertyInfo> idProps, IDbConnection connection, object whereConditions = null)
        {
            ...
            for (var i = 0; i < propertyInfos.Count(); i++)
            {
                ...
                sb.AppendFormat(
                    ...
                    GetColumnName(propertyToUse, connection),
                    GetDialect(connection, DBDialectOption.ParamPrefix),
                    propertyToUse.Name);
        ...
        private static void BuildInsertValues<T>(StringBuilder masterSb, IDbConnection connection)
        {
            StringBuilderCache(masterSb, $"{typeof(T).FullName}_BuildInsertValues", sb =>
            {
                ...
                for (var i = 0; i < props.Count(); i++)
                {
                    ...
                    sb.AppendFormat("{0}{1}", GetDialect(connection, DBDialectOption.ParamPrefix), property.Name);
                    if (i < props.Count() - 1)
                        sb.Append(", ");
                }
        ...
        private static void BuildInsertParameters<T>(StringBuilder masterSb, IDbConnection connection)
        {
        ...
        private static string GetTableName(object entity, IDbConnection connection)
        {
            var type = entity.GetType();
            return GetTableName(type, connection);
        }
        ...
        private static string GetTableName(Type type, IDbConnection connection)
        {
            ...
            tableName = _tableNameResolver.ResolveTableName(type, connection);
        ...
        private static string GetColumnName(PropertyInfo propertyInfo, IDbConnection connection)
        {
            ...
            columnName = _columnNameResolver.ResolveColumnName(propertyInfo, connection);
        ...
        private static string Encapsulate(string databaseword, IDbConnection connection)
        {
            return string.Format(GetDialect(connection, DBDialectOption.Encapsulation), databaseword);
        }
        /// <summary>
        ...
        {
            string ResolveTableName(Type type, IDbConnection connection);
        }

        public interface IColumnNameResolver
        {
            string ResolveColumnName(PropertyInfo propertyInfo, IDbConnection connection);
        }

        public class TableNameResolver : ITableNameResolver
        {
            public virtual string ResolveTableName(Type type, IDbConnection connection)
            {
                var tableName = Encapsulate(type.Name, connection);

                var tableattr = type.GetCustomAttributes(true).SingleOrDefault(attr => attr.GetType().Name == typeof(TableAttribute).Name) as dynamic;
                if (tableattr != null)
                {
                    tableName = Encapsulate(tableattr.Name, connection);
                    try
                    {
                        if (!String.IsNullOrEmpty(tableattr.Schema))
                        {
                            string schemaName = Encapsulate(tableattr.Schema, connection);
        ...
        {
            public virtual string ResolveColumnName(PropertyInfo propertyInfo, IDbConnection connection)
            {
                var columnName = Encapsulate(propertyInfo.Name, connection);

                var columnattr = propertyInfo.GetCustomAttributes(true).SingleOrDefault(attr => attr.GetType().Name == typeof(ColumnAttribute).Name) as dynamic;
                if (columnattr != null)
                {
                    columnName = Encapsulate(columnattr.Name, connection);
                    if (Debugger.IsAttached)
